...
TokenStrategy.prototype.authenticate = function(req) {
  ...
  if (req.query && req.query['oauth_signature']) {
    ...
    // In the event + char was passed as part of signature and automatically
    // converted to a space, convert it back
    params['oauth_signature'] = params['oauth_signature'].replace(/\s/g, '+');
  }
  ...
  var consumerKey = params['oauth_consumer_key']
    ...
    , signature = decodeURIComponent(params['oauth_signature'])
